...
{	
	// Translate IP address info from socket to string
	char * ipString = ipAddressToString(ipAddressStruct);
	
	// Print IP number as a string and the port in host order
	printf("IP: %s Port: %d\n", ipString, ntohs(ipAddressStruct->sin6_port));
	
}

// Translate IP Address struct to string
...
// Return IP address of host via the host name
