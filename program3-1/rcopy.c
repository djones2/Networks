...
// MISSING = RECOVERY
enum State {
	DONE, FILENAME, RECV_DATA, FILE_OK, START_STATE, RECOVERY
};
...
STATE recovery(Window *window, int32_t output_file);
void checkArgs(int argc, char ** argv);
void srej(int seq_num);
void rr(int seq_num);
...
void processFile(char * argv[]) {
	...
	Window window;
	create_window(&window, atoi(argv[5]));
	while (state != DONE)
	{
		switch (state)
		{
...
STATE filename(char * fname, int32_t buf_size, Connection * server) {
	...
	{
		recv_check = recv_buf(packet, MAX_LEN, server->socket_number, server, &flag, &seq_num);
...
STATE recv_data(int32_t output_file, Connection *server, uint32_t * clientSeqNum) {
	...
	data_len = recv_buf(data_buf, MAX_LEN, server->socket_number, server, &flag, &seq_num);
...
void checkArgs(int argc, char * argv[])
{
	...
	{
		printf("From-filename too long - needs to be less than 1000 and is %zu\n", strlen(argv[1]));
		exit(-1);
	}
	if (strlen(argv[2]) > 1000)
	{
		printf("To-filename too long - needs to be less than 1000 and is %zu\n", strlen(argv[2]));
		exit(-1);
	}
